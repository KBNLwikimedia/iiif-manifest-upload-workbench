...
The **wizard-polish, duplicate-reporting & Codex-layout** milestone. The import wizard now surfaces manifests that need checking and lets you file them as `manifest-needs-checking` GitHub issues from inside the tool (OI-85), keys the recent-manifest list on each manifest's canonical `id` (OI-88), and hardens the report-modal GitHub calls (OI-90). Every modal is now draggable and resizable (OI-83); Commons filenames are capped by UTF-8 bytes, not characters (OI-29). A consistent "Short title — signature" identity runs through the header, hover popups and lightbox, and the CC0 + 48-hour-waiver onboarding is a two-step flow shown every session. Plus a Codex layout pass pulling the main workbench toward the Wikimedia look (quiet topbar Feedback, centred 1200px column, de-carded blocks, unified tiles), a `.json`-drop import target, a lightbox-following carousel, and a new `npm test` unit harness.

### Fixed

- **Report-modal GitHub calls hardened (OI-90 #90)** — the "Find my issue" search now strips double quotes from the manuscript signature/title before embedding it in the quoted `in:title "…"` qualifier, so a title containing a `"` can't break the query; the "newest issue" baseline is cached for the session so repeated report-modal opens don't each spend an unauthenticated core-API request.
- **Recent-manifest list keys on the manifest's canonical identity, not the load URL (OI-88 #88)** — a manifest is now identified by its self-declared `id` (host-independent, the same whether it was loaded by typed URL or dropped as a `.json` file) instead of the exact URL string it happened to arrive on. Loading the same manuscript two ways no longer creates two split entries; the "needs work" flag and recorded GitHub issues merge onto one row, and re-loading carries the reported-issue links over. Legacy entries written before this change are superseded (not duplicated) on next load. When a dropped file has no reusable URL at all, the report modal now says so and hides "Save issue #" instead of silently dropping the number. The recent row also **displays the canonical `id`** and classifies the manifest by it — so a KB manuscript loaded via a mirror URL shows its real KB URL and lands in the **KB** tab (it still re-loads from the URL you actually used).
...
- **Codex layout pass** — a set of changes pulling the main workbench toward the Wikimedia/Codex look (per `DESIGN.md`). *Tier 1:* the always-floating blue "Feedback" pill becomes a quiet button in the topbar next to *About* (no floating FAB, "one accent per screen"); the content is capped at a **centred 1200px column** with a 24px min gutter (full-width bars keep their edge-to-edge background but align content to the column). *Tier 2 (de-card):* the stash table and the "hidden files" collapsible become **rule-separated blocks** (hairline top/bottom rules, no rounded card / dashed borders) instead of cards floating on the page; **"Clear entire stash"** now reads as the destructive action it is (red) rather than grey text blending into the metadata line. *Tiles unified:* the **upload-history grid tiles** and the **import-wizard select-step tiles** adopt the stash Card design — an edge-to-edge thumbnail (4/3 for history, 3/4 full-page for the select step) with a framed card and a semibold title; the select tiles keep their selection features (checkbox, number, badges, dim-unselected). *Polish:* Feedback + About tooltips say the full "IIIF Manifest Upload Workbench"; the account avatar is neutral chrome (hairline circle, dark initials) instead of a blue pill; the import-wizard modal default width drops to `min(880px, 94vw)` for the text steps, staying wide (1480px) only on the select step.
- **Obsolete consent keys are cleaned out of `Preferences.json`** — now that neither the CC0 nor the 48-hour waiver is persisted, any leftover `cc0Acknowledgment` / `iiifWaiver` records are stripped on load and the wiki page is rewritten without them on the next save (one-shot migration, same mechanism as the old `sha1Index` cleanup). No dedicated extra write.
- **CC0 consent is asked every session and no longer stored** — the "don't remind me again" option is gone; the CC0 notice (step 1 of the onboarding) shows on every fresh session with a single **"OK, I understand"** button, the step is a "Step 1/2 -" prefix on the title, the × / Esc / backdrop dismissal is removed (the button is the only way through, so it can't be skipped by accident), and nothing is written to `Preferences.json` for it (`cc0Acknowledgment` is no longer read or written).
...
- **The CC0 and waiver "OK, I understand" buttons sit at the exact same spot** — the two coupled onboarding modals share a width and a min-height, so their footers (and the OK button) land at the same X/Y; advancing step 1 → 2 needs no mouse movement to click OK again.
...
- **Removing a recent manifest asks first** — the per-item × (with the manuscript name) and "Clear all" (with the count) now show a confirmation before deleting, matching the app's existing `window.confirm` pattern.
...
<details>
<summary><strong>Commits in this release (56)</strong> — v0.41.0…v0.42.0 (newest first, with commit timestamps)</summary>

- `2026-07-12 20:16` [`3205b97`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/3205b9710cd5147bc6decc6c9c45392f0caaa821) release: v0.42.0 — wizard-polish, duplicate-reporting &amp; Codex-layout milestone
- `2026-07-12 20:03` [`dbd8103`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/dbd810330a336fb2342f72a27b356e9ec1d1cddd) chore: remove stray test images from the repo root
- `2026-07-12 19:56` [`ad2714e`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/ad2714e992f218dde9fcafa580df2852bc03e5ef) docs: sweep — reconcile CHANGELOG + CLAUDE.md for the 2026-07-12 round
- `2026-07-12 19:44` [`04dc0f5`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/04dc0f51468c05302a8b644729cc37b4295b90d9) feat: select-step tiles adopt the stash card design (full-page thumb)
- `2026-07-12 19:33` [`48d402f`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/48d402f9fb2fb8ffe624ee1d21fad088fe34ba49) feat: upload-history grid tiles adopt the stash card design
- `2026-07-12 19:24` [`c0690ae`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/c0690ae3866f55ad1bfc2c197b20a485604caac9) feat: Codex layout tier 2 — de-card content into rule-separated blocks
- `2026-07-12 19:13` [`d198164`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/d19816423079cd301cb1643391a4c890ad5bd94e) fix: align the OK button across the CC0 and waiver steps (same screen spot)
- `2026-07-12 19:10` [`e948a4c`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/e948a4c1f0fa15663f841af48dfed56999570de0) fix: full product name in tooltips, neutral account avatar, right-size wizard modal
- `2026-07-12 19:01` [`c1f6b7a`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/c1f6b7a20b21f25283301d9f16e95fde9841d3c4) feat: Codex layout pass tier 1 — feedback in topbar + 1200px content column
- `2026-07-12 17:41` [`0c8eb82`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/0c8eb8282f50a59c5787d1f0854f1048a403dd97) docs: add DESIGN.md (Wikimedia Codex design reference)
- `2026-07-12 17:41` [`263bacc`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/263baccfebbd58a37941b8333c4dcc5c1cb3bc41) refactor: tech-debt sweep (OI-89) + harden report-modal GitHub calls (OI-90)
- `2026-07-12 17:31` [`14faaf5`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/14faaf5e675b4902e609842ea4ce45b8f337bcd5) feat: recent list shows the canonical manifest id, classifies by it (OI-88 follow-up)
- `2026-07-12 17:13` [`763817e`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/763817ebd16703da06b6ebf8094234600c36fd28) feat: confirm before removing a recent manifest (× and Clear all)
- `2026-07-12 17:06` [`e5bc2fa`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/e5bc2fa82328d1f3b4e3203643f2e8c0b8fd1778) fix: key recent-manifest list on canonical manifest id, not load URL (OI-88)
- `2026-07-12 14:50` [`9ee6cc8`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/9ee6cc87acf62ad949a240ab942816764b0de7a2) docs: exempt manifest-needs-checking reports from the OI-NN title convention
- `2026-07-12 14:43` [`2d458eb`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/2d458ebaee34c0aff8177222c782feeb053c7e97) fix: key reported issues on the loaded manifest, not the URL input + review round
- `2026-07-12 14:17` [`71ccfb0`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/71ccfb08184613fdc8df8822a68be536cebb7eb9) chore: strip obsolete cc0Acknowledgment/iiifWaiver from Preferences.json
- `2026-07-12 14:09` [`cee63c0`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/cee63c0bf3511c756987a9026930d85cb342154c) fix: CC0 modal dismissible only via its button (no Esc / backdrop)
- `2026-07-12 14:07` [`3d41ae3`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/3d41ae3a4edd39007fbc1fa6dd6ab7ccec2693eb) feat: step prefix in onboarding titles, drop CC0 close cross
- `2026-07-12 14:03` [`9357b43`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/9357b431ac6435caca240d81fc8fbf099ec193bd) feat: waiver also asked every session (drop checkbox + persistence), matching OK buttons
- `2026-07-12 13:58` [`acce285`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/acce285d59b0e9909ae6a6de12d76314e0b8699f) feat: CC0 consent every session, no persistence (drop don't-remind option)
- `2026-07-12 13:53` [`b584f08`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/b584f0840e74981c956c68f97d11cb9cf38375d2) feat: couple CC0 + waiver as a two-step flow with a Back button
- `2026-07-12 13:48` [`8aa7f22`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/8aa7f225904d93a1a69291d284c570938df87d68) fix: waiver = one button (re-show when unchecked), sticky bar flush to header
- `2026-07-12 13:38` [`0b1705b`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/0b1705b7e43ddddfe8348fc2a38fb327cda63df7) feat: selectable modal headers, header needs-work flag, sticky select bar, button order
- `2026-07-12 13:27` [`6951c8e`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/6951c8e97bf8ea776d9aac5baf87d2849c948e67) feat: 48-hour upload waiver modal, digit-only issue input, sign-in copy
- `2026-07-12 13:16` [`851e646`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/851e646cec19126d62c8650479dea2d14f945859) feat: whole-body scroll on select step + reword step-2 record label
- `2026-07-12 13:10` [`6bdea85`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/6bdea853375d9706d802263320e8d71eecdc1d96) feat: validate the recorded issue number (no dupes, must be a new issue)
- `2026-07-12 13:01` [`d879c02`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/d879c02657dfeab25384932805c9d7b4056e062a) feat: remove a wrongly-recorded issue number and look up again
- `2026-07-12 12:56` [`f16aca0`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/f16aca05b807d731f3e6fb3e285fd2ab9595af20) feat: order report modal as create-then-record (numbered steps)
- `2026-07-12 12:53` [`1d03e5e`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/1d03e5eb704ccaf53ff8b0bad3733ff304327f2f) feat: turn "Find my issue" button red when no issue is found
- `2026-07-12 12:51` [`4d3a8aa`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/4d3a8aac05e8494437d0a08423878bae699d67ad) fix: dim disabled buttons (base .btn:disabled style)
- `2026-07-12 12:39` [`81621ed`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/81621ed6536ea7a21de69d88fff21a4810e79d98) feat: "Find my issue" auto-suggest + report button border
- `2026-07-12 12:26` [`f378e77`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/f378e7748eb7cc99e2e07e9970b8669b664c4828) feat: flag + report manifests with duplicates (OI-85 needs-work)
- `2026-07-12 04:37` [`e43cc72`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/e43cc72174a676dfa1982bfc20dcb5e69c030393) feat: show "Step N/3" in the import wizard titles
- `2026-07-12 04:35` [`9a13847`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/9a13847475858e189233b19d33730477e89b44af) feat: number the import wizard steps in the modal titles
- `2026-07-12 04:33` [`b553075`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/b5530750be97baf9d169694dc574641f898dd62c) feat: position badge in the lightbox image top-left
- `2026-07-12 04:30` [`87798d9`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/87798d9ecbf720589acd1e640fbaafa3cd15154c) feat: sync review carousel to lightbox position on navigate
- `2026-07-12 04:20` [`d3d2c70`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/d3d2c708fe3434d8985c75755eb2ef54a1866fe9) fix: put header thumbnail in front of the identity line, keep app logo
- `2026-07-12 04:17` [`1c0b02e`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/1c0b02ebe5d11e619d02f102d91871fa91728609) feat: manifest first-image thumbnail in the wizard header
- `2026-07-12 04:14` [`47b8d7a`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/47b8d7adb13d0c287a3d4427ef20c7d68b14306b) docs: mention drag-and-drop in the import-step subtitle
- `2026-07-12 04:13` [`e0a3eef`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/e0a3eeff5a2560e3d42f041001fe613704546d55) feat: drop a manifest .json onto the import modal to load it
- `2026-07-12 04:08` [`9f2ded0`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/9f2ded079ae81bec656375bebff64b802d3792c6) feat: soft cap of 200 on the recent-manifest list
- `2026-07-12 04:05` [`1bd6725`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/1bd6725335fe42587e679f9167ad8b2c037b82b6) feat: unbounded recent-manifest list (drop 10-entry cap)
- `2026-07-12 03:59` [`60aa9d1`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/60aa9d16a3530e106352c1d3b088465875503c5e) feat: show "Short title — signature" in select-step hover popup
- `2026-07-12 03:58` [`40ee09f`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/40ee09f6037492cb00bf9404a32d7685f0da8bdb) feat: add signature to lightbox carousel caption
- `2026-07-12 03:50` [`fb478ea`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/fb478eaa734c9c5fc190ff2ec3a93f2932a00787) feat: hover-preview caption on top with filename + short title
- `2026-07-12 03:46` [`e422b30`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/e422b301fb4f769c7361acaa25d3987f0f1b5746) feat: prominent Short title + signature in wizard header (steps 2–4)
- `2026-07-12 00:56` [`82b9527`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/82b952793050941420697d1b29b552ac11dd4b2b) feat: byte-cap Commons filenames (OI-29), recent-list width, + full docs sweep
- `2026-07-12 00:44` [`7f92838`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/7f92838ce8872da978464a0733530368886917b1) Fix: only cap the lightbox title for a summary-fallback, not by length
- `2026-07-12 00:36` [`a00eecb`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/a00eecba1f9bd803922c2fec39e24cb1abd22069) Fix: cap the lightbox manuscript title so a summary-fallback doesn't fill it
- `2026-07-12 00:30` [`a6a821b`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/a6a821bc79b3fed20f46c1aea453572e39157d83) feat: show the manuscript title in the review-step lightbox caption
- `2026-07-12 00:19` [`c4d2a2b`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/c4d2a2ba8b310fd8d3dd3889d6b94d8545dfbda1) feat: right-hand resize gutter beside the modal scrollbar (OI-83)
- `2026-07-12 00:13` [`a0c1242`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/a0c1242794e826765d785bef467e7b1210587a9c) feat: title-bar modal headers + resize grabbable outside the edge (scrollbar fix)
- `2026-07-12 00:05` [`d37fa25`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/d37fa251c2214321a09644399292e718eaa8ddcd) feat: resizable modals — drag any edge/corner, opposite edge anchored (OI-83)
- `2026-07-11 23:56` [`a60c300`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/a60c30050737d82ad347d53c89ae95223fa7fa09) docs: OI-83 (draggable modals) → Closed table + header/memory
- `2026-07-11 23:55` [`3a49bf2`](https://github.com/KBNLwikimedia/iiif-manifest-upload-workbench/commit/3a49bf20d9fbc39a37e4b4ab2d3c4be546e49871) feat: draggable modals — grab the header to move any dialog aside (OI-83)
