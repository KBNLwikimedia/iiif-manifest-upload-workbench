...
- **48-hour upload waiver modal** — shown right after the CC0 notice, it asks the user to actively acknowledge that imported images are staged in a temporary Commons upload area that is **cleared after 48 hours**, so the import→publish flow must be finished within that window. Two genuinely distinct outcomes (both proceed — it's a warning, not a hard gate): **Accept** records `iiifWaiver.accepted = true` and is never shown again; **Decline** ("proceed at my own risk") records `accepted = false` and re-appears next session to nudge acceptance — unless the **"Don't show this again"** checkbox (unchecked by default) is ticked, which sets `suppressFurther`. Persisted as `iiifWaiver` in `Preferences.json`.
