...
- **Codex layout pass, tier 1** — two changes that pull the main workbench closer to the Wikimedia/Codex look (per `DESIGN.md`): (1) the always-floating blue "Feedback" pill is gone — Feedback is now a quiet button in the topbar's right cluster next to *About*, so it no longer floats over the header or competes with the primary blue CTA ("one accent per screen"); (2) the app content is capped at a **centred 1200px column** with a 24px minimum gutter instead of stretching edge-to-edge — the full-width topbar/toolbar keep their edge-to-edge background but align their content to the same column.
...
- **Modal button order** — the CC0 dialog leads with its primary button ("don't remind me again"); the waiver dialog's single "OK, I understand" button is centered.
