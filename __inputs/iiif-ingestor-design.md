...
2. **Manifest URL scheme changed** — manifests moved from `…/32/middeleeuwse-manuscripten/<slug>` to `…/32/<slug>`. The collection index `https://presentation-api.dlc.services/32` lists all 29 manifests + 4 sub-collections, machine-readably. KB announced a future move to `iiif.kb.nl` (not live yet). ⇒ never hard-code the base URL; treat any pasted/fetched manifest URL as opaque.
...
### Phase 3 — Metadata mapping (`src/api/iiif-map.js`) — *the heart of it*
| # | Task | Depends on |
|---|---|---|
